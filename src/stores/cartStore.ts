  ...
  persist(
    (set, get) => ({
      ...
      removeItem: async (variantId) => {
        ...
        set({ isLoading: true });
        try {
          const result = await removeCartLine({ data: { cartId, lineId: item.lineId } });
          if (result.success) {
            const currentItems = get().items;
      ...
      syncCart: async () => {
        ...
        set({ isSyncing: true });
        try {
          const cart = await syncCartServer({ data: { cartId } });
          if (!cart) return;
